...
Date: 2024-07-20 13:20:11
LastEditors: Guosy_wxy 1579528809@qq.com
LastEditTime: 2024-07-21 01:56:26
FilePath: /Prompt/mvlpt-master/scripts/datasets/office_home.py
...
class OfficeHome(DatasetBase):
    ...
    train_domains = ["art", "clipart"]
    val_domains = ["product"]
    test_domains = ["real_world"]

    def __init__(self, cfg):
        root = osp.abspath(osp.expanduser(cfg.DATASET.ROOT))
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.image_dir = osp.join(self.dataset_dir, "Real_World")
        self.split_path = osp.join(self.dataset_dir, "split_zhou_office-home_Real_World.json")
        ...
        if cfg.DATASET.SOURCE_DOMAINS and cfg.DATASET.TARGET_DOMAINS:
            train = self._read_data(cfg.DATASET.SOURCE_DOMAINS)
            val = self._read_data(cfg.DATASET.TARGET_DOMAINS)
