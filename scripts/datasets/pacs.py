...
class PACS(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        self.image_dir = osp.join(self.dataset_dir, "real")
        self.split_dir = osp.join(self.dataset_dir, "splits")
        self.split_fewshot_dir = osp.join(self.dataset_dir, "split_fewshot")
        mkdir_if_missing(self.split_fewshot_dir)

        train = self._read_data(["real"], split="train")
        val = self._read_data(["real"], split="test")
        test = self._read_data(["real"], split="test")
    ...
    def _read_data(self, input_domains, split="train"):
        ...
        if split == "train":
            lines_num = 22506#34516
        else:
            lines_num = 9701#14873
