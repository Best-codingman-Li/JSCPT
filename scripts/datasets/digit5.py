...
LastEditTime: 2024-07-21 02:44:49
...
class Digit5(DatasetBase):
    """Digit5 to process the art task data in Office-home.
    ...
    def __init__(self, cfg):
        ...
        self.image_dir = osp.join(self.dataset_dir, "Art")
        self.split_path = osp.join(self.dataset_dir, "split_zhou_office-home_art.json")
        self.split_fewshot_dir = osp.join(self.dataset_dir, "split_fewshot")
        mkdir_if_missing(self.split_fewshot_dir)

        if osp.exists(self.split_path):
            #print("OxfordPets.read_split")
            train, val, test = OxfordPets.read_split(self.split_path, self.image_dir)
            #print(len(train),len(val),len(test))
        else:
            #print("DTD.read_and_split_data")
