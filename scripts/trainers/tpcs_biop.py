...
from dassl.engine import TRAINER_REGISTRY, Trainer_Bli_level
...
class MTVLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        #self.coop_mask = MaskMLP(128)
        #self.vpt_mask = MaskMLP(128)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, dm=None):
        ...
        self.dtype = clip_model.dtype
        self.prompt_learner = MTVLPromptLearner(cfg, classnames, clip_model)
    ...
    def CustomCLIP_Logits(self, image_features, text_features):
        #print("image_features.device", image_features.device)
    ...
    def forward(self, image, task=None, train=True):
        coop_emb, vpt_emb, vpt_emb_deep = self.prompt_learner.forward_mvlpt_proj(self.dtype) #vpt_emb.size()=[1, 16, 768];vpt_emb_deep.size()=[11, 16, 768];coop_emb.size()=[16,512]
        ...
        #print("coop_emb.device=", coop_emb.device)
        #print("vpt_emb.device=", vpt_emb.device)
        #print("vpt_emb_deep.device=", vpt_emb_deep.device)

        image_features = self.image_encoder(image.type(self.dtype), vpt_emb, vpt_emb_deep) #torch.Size([bs, 512]
        #print("image_features.device=", image_features.device)

        cau_image_features = self.image_mask[task](image_features)
        #print("cau_image_features.device=", cau_image_features.device)
        ...
        if self.prompt_learner.cocoop_ctx == None:
            ...
            if train:
                cf_logits = self.CustomCLIP_Logits(image_features, text_features)
...
class MVLPT(Trainer_Bli_level):
    ...
    def build_model(self):
        ...
        print(f"tpcs_relu_1e3_all_pa Tunable Param: {sum([p.numel() for p in self.model.parameters() if p.requires_grad])/10**6}M, Original CLIP {sum([p.numel() for p in self.model.parameters() if not p.requires_grad])/10**6}M")
        
        #self.tune_param_set = [param for param in self.model.parameters() if param.requires_grad] 
        self.tune_param_set =  list(self.model.prompt_learner.parameters()) + list(self.model.image_mask.parameters())
        ...
        self.register_model("tpcs_relu_1e3_all_pa", self.model, self.optim, self.sched)
        ...
        if device_count > 1:
            ...
            #self.aux_model = nn.DataParallel(self.aux_model)
        else:
    ...
    def forward_backward(self, batch_list, val_batch_list):
        ...
        for task_index, batch in enumerate(batch_list):
            ...
            #causal_effect[task_index] = torch.exp(cf_task_loss_list[task_index] - task_loss_list[task_index])
            causal_effect[task_index] = (cf_task_loss_list[task_index] - task_loss_list[task_index]) / cf_task_loss_list[task_index]
            #total_loss_list[task_index] = torch.sigmoid(causal_effect[task_index]) * task_loss_list[task_index] + (1 - torch.sigmoid(causal_effect[task_index])) * cf_task_loss_list[task_index]
        ...
        #self.model_backward_and_update(final_loss)
        ...
        if (self.batch_idx > 0) and (self.batch_idx % self.hyperstep == 0):
            ...
            for task_name, val_batch in enumerate(val_batch_list):
                ...
                if prec == "amp":
                    with autocast():
                        output, _ = self.model(val_image, task=task_name, train=True)
                    ...
                    val_output, _ = self.model(val_image, task=task_name, train=True)
            ...
            for in_batch_idx in range(self.in_t_batch_nums):
                if n_train_step < n_meta_train_loss_accum:
                    ...
                    for task_name in self.train_task_name_list:
                        ...
                        if prec == "amp":
                            with autocast():
                                output, _ = self.model(in_train_image, task=task_name, train=True)
                        ...
                        #in_causal_effect[task_name] = torch.exp(in_train_cf_loss_list[task_name] - in_train_loss_list[task_name])
                        in_causal_effect[task_name] = (in_train_cf_loss_list[task_name] - in_train_loss_list[task_name]) / in_train_cf_loss_list[task_name]
                        #in_total_loss_list[task_name] = torch.sigmoid(in_causal_effect[task_name]) * in_train_loss_list[task_name] + (1 - torch.sigmoid(in_causal_effect[task_name])) * in_train_cf_loss_list[task_name]
            ...
            #print("inner_meta_train_loss=", inner_meta_train_loss)
            #print("list(self.aux_model.parameters())=", list(self.aux_model.parameters()))

            self.meta_optimizer.step(val_loss = meta_val_loss, train_loss = inner_meta_train_loss, parameters = list(self.model.parameters()), aux_params = list(self.aux_model.parameters()))#parameters = self.tune_param_set
            self.aux_model.clamp()
    ...
    def _grad_calculate(self, task_loss_list, model_para_set):
        ...
        cossim_list=[[] for _ in range(self.task_nums)]
        magsim_list=[[] for _ in range(self.task_nums)]
        ...
        for idx_i in range(self.task_nums):
            for idx_j in range(idx_i+1, self.task_nums):
                ...
                magsim = magnitude_sim(grad_list[idx_i],grad_list[idx_j])
                cossim_list[idx_i].append(cossim)
                cossim_list[idx_j].append(cossim)
