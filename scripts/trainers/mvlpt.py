...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, dm=None):
        ...
        print("the num of tasks======", len(dm._task_names))
        print("dm._task_names=====", dm._task_names)
        print("dm._num_classes=======", dm._num_classes)        
        '''
    ...
    def forward(self, image, task=None):
        ...
        '''
        coop_emb_ = torch.cat((coop_emb, coop_emb), dim=0).reshape(self.coop_n_ctx, -1)
        coop_emb = self.coop_linear(coop_emb_)

        vpt_emb_ = torch.cat((vpt_emb, vpt_emb), dim=1).reshape(self.vpt_n_ctx, -1)
        vpt_emb = self.vpt_linear(vpt_emb_).unsqueeze(0)

        vpt_emb_deep_ = torch.cat((vpt_emb_deep, vpt_emb_deep), dim=1).reshape(11, -1)
...
class MVLPT(TrainerXX):
    ...
    def build_model(self):
        ...
        print(f"MVLPT Tunable Param: {sum([p.numel() for p in self.model.parameters() if p.requires_grad])/10**6}M, Original CLIP {sum([p.numel() for p in self.model.parameters() if not p.requires_grad])/10**6}M")
        ...
        self.register_model("MVLPT", self.model.prompt_learner, self.optim, self.sched)
...
class MVLPT(TrainerXX):
    ...
    def parse_batch_train(self, batch):
        ...
        #task_image_dic, task_label_dic = {}, {}
        #task_l = tasks.tolist()
        #task_name_set = [0, 1, 2]
        #print("***************")
        #print("task_name_set", task_name_set)
       # for task_name in task_name_set:
         #   task_image_dic[task_name], task_label_dic[task_name] = [], []
        #for i in range(len(label)):
            #print("tasks[i]", tasks[i])
         #   if task_l[i] in task_name_set:
          #      task_image_dic[task_l[i]].append(input[i, :, :, :])
         #       task_label_dic[task_l[i]].append(label[i])

        #for key, value in task_image_dic.items():  
         #   print(key)
         #   print(len(value))
    ...
    def test(self, split=None):
        ...
        for task in self.evaluator_task:
            ...
            if self.cfg.DATASET.COOP:
                ...
                results = { self.dm._metric_name[task]: metric_result }
                results_overall[ task ] = metric_result
            print( 'evaluator_task results=====', results )
            for k, v in results.items():
                tag = f"{split}/{task}/{k}"
                self.write_scalar(tag, v, self.epoch)
        ...
        if self.multi_task:
            ...
            if not self.cfg.DATASET.COOP:
                ...
                results = self.evaluator.evaluate()
        print( 'Overall evaluation results', results )
        for k, v in results.items():
            tag = f"/{split}/{k}"
            self.write_scalar(tag, v, self.epoch)
